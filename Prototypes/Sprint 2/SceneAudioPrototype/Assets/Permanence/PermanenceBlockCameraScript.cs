...
public class PermanenceBlockCameraScript : MonoBehaviour
{
    ...
    [SerializeField]
    private AudioSource[] bleepBloopAudios;
    public GameObject BounceSFXobj;
    
    private float[] timeTillAdd;
    
    private float timeSinceLastCam = 0;
    void Start()
    {
        ...
        for(int i = 0; i < armatures.Length; i++) {
            ...
            timeTillAdd[i] = UnityEngine.Random.Range(1f, 2.5f);
        }
    }

    private void CamAudio(){
        if (timeSinceLastCam >= 5f){
            timeSinceLastCam = 0;
            bleepBloopAudios[UnityEngine.Random.Range(0,2)].Play();
        }
    }
    // Update is called once per frame
    void Update() {
        addAnimations();
        timeSinceLastCam += Time.deltaTime;
        for(int i = 0; i < armatures.Length; i++) {
    ...
    private void addAnimation(int cam) {
        float target = UnityEngine.Random.Range(overshootAmmount, 1f-overshootAmmount);
        ...
        timeTillAdd[cam] = UnityEngine.Random.Range(0.5f, 1f);
    }
